...
$\text{RA} = \text{Riesgo en expuestos} - \text{Riesgo en no expuestos}$
$\text{RA} = \text{𝐼𝑛𝑐𝑖𝑑𝑒𝑛𝑐𝑖𝑎 𝑒𝑛 𝑛𝑜 𝑒𝑥𝑝𝑢𝑒𝑠𝑡𝑜𝑠 × (𝑅𝑅 − 1)}$
...
Interpretación:
Si el tratamiento para reducir la hipertensión fuera completamente efectivo, se podría eliminar el 60% del riesgo de eventos cardiovasculares en la población evaluada, bajo la suposición de que no habría otros factores de riesgo involucrados.

Consideraciones importantes:
La FER solo tiene sentido en situaciones donde hay una relación causal directa entre la exposición y el desenlace.
